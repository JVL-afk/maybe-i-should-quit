...
import DashboardSidebar from '../../../components/dashboard/sidebar' // Corrected relative path
...
interface Website {
  ...
  url: string;
  createdAt: string;
  status: 'active' | 'building' | 'paused' | 'failed'; // Example statuses
  traffic: number;
  clicks: number;
  revenue: number;
  category: string;
...
export default function MyWebsitesPage() {
  const [filter, setFilter] = useState<Website['status'] | 'all'>('all')
  const [sortBy, setSortBy] = useState<keyof Website>('createdAt')
  ...
  return (
    <div className="flex min-h-screen">
      <DashboardSidebar /> {/* Added Sidebar */}
      <main className="flex-1 p-8">
        ...
        <div className="flex flex-col md:flex-row justify-between mb-6 gap-4">
          <div className="flex items-center space-x-4">
            ...
            <select
              value={filter}
              onChange={(e) => setFilter(e.target.value as Website['status'] | 'all')}
          ...
          <div className="flex items-center space-x-4">
            ...
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as keyof Website)}
              className="bg-[rgba(45,26,61,0.7)] border border-[rgba(255,255,255,0.1)] rounded-md px-3 py-1 text-sm"
            >
